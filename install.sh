...
echo " |  ____|/ ____|  __ (_)   | |    V1.0      | |  github/wzern  | |"
...
cp ./conf/pma.conf /etc/dbconfig-common/phpmyadmin.conf &>/dev/null
dpkg-reconfigure --frontend=noninteractive phpmyadmin &>/dev/null
cp ./conf/phpmyadmin.conf /etc/apache2/conf-available/phpmyadmin.conf &>/dev/null
cp ./conf/phpmyadmin.conf /etc/apache2/conf-enabled/phpmyadmin.conf &>/dev/null
